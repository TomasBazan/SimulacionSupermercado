...
def actualizar_estado_reparacion(tiempo_reparacion1,
                                 tiempo_reparacion2,
                                 lista_de_eventos,
                                 t, r, TR
                                 ):
    # Reviso cual de los operarios reparo una maquina, disminuyo las maquinas
    # descompuestas y actualizo el tiempo al tiempo de reparacion.
    r -= 1
    t = min(tiempo_reparacion1, tiempo_reparacion2)
    if tiempo_reparacion1 <= lista_de_eventos[0]:
        # Si no tengo maquinas para reparar o si el otro operario esta
        # reparando una maquina
        if r == 0 or (r == 1 and tiempo_reparacion2 != np.inf):
...
def actualizar_estado_ruptura(tiempo_reparacion1,
                              tiempo_reparacion2,
                              lista_de_eventos,
                              t, r, TR, TF
                              ):
...
def reparacion_2_operarios(N, s, TF, TR):
    ...
    lista_de_eventos = []
    t, r = 0, 0
    tiempo_reparacion_operario1 = np.inf
    tiempo_reparacion_operario2 = np.inf
    for _ in range(N):
        lista_de_eventos.append(simular_Tbreak(TF))
    lista_de_eventos.sort()

    while True:
        if (tiempo_reparacion_operario1 <= lista_de_eventos[0] or
                tiempo_reparacion_operario2 <= lista_de_eventos[0]):
            # Caso se repara alguna maquina
            tiempo_reparacion_operario1, tiempo_reparacion_operario2, t, r = actualizar_estado_reparacion(tiempo_reparacion_operario1, tiempo_reparacion_operario2, lista_de_eventos, t, r, TR)

        elif (tiempo_reparacion_operario1 > lista_de_eventos[0] and
              tiempo_reparacion_operario2 > lista_de_eventos[0]):
            # Caso de ruptura de alguna maquina
            t = lista_de_eventos[0]
            # No tengo mas maquinas de repuesto
            if r == s:
                return t
            # Tengo maquinas de repuesto,
            # veo a que operario asignarle la reparacion
            else:
                tiempo_reparacion_operario1, tiempo_reparacion_operario2, t, r = actualizar_estado_ruptura(tiempo_reparacion_operario1, tiempo_reparacion_operario2, lista_de_eventos, t, r, TR, TF)
...
def simulacion_ejercicio1(Nsim):
    ...
    resultados = []
    for _ in range(Nsim):
        sim = reparacion_1_operario(7, 4, 1, 8)
        x.append(sim)
        x_cuad.append(sim**2)
